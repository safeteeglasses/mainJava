...
export default {
  ...
  methods: {
    ...
    deleteReview(id) {
      fetch(`${this.apiURL}/${id}`,{   // Asynchonously call api with id of review to delete
        method: 'DELETE',              // Call api with http DELETE
        headers: {
          "Content-Type": "application/json" // Tell api data is in application/json format
        },
        body: JSON.stringify(this.review)    // Convert JavaScript object to json in request body
      })
      .then((response) => {
        if(response.ok) {
  ...
  created() {     // method to be run when the Vue created hook is activated
    // load the reviews
    fetch(this.apiURL)         // Asynchrously call this api
      .then((response) => {    // Wait for fetch to get all the data
        return response.json();// Convert the data to json format
      })
      .then((reviews) => {     // Wait for data returned from fetch to be converted to json
        this.reviews = reviews;// Assign json data from fetch to reviews
      })
      .catch((err) => console.error(err));
  }
};
